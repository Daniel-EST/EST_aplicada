...
chisq.test(m,correct=F)
ggplot(dados,aes(x=satisflimite))+geom_bar(aes(fill=serasa))+ylab("Frequência")+
...
#Fazer descritivas do gerente por satisfação####
...
chisq.test(m,correct=F)
ggplot(dados,aes(x=satisflimite))+geom_bar(aes(fill=gerente))+ylab("Frequência")+
  xlab("Possui nome no SERASA")
